...
<section id="approach" class="py-16 px-4 sm:px-6 lg:px-8 bg-dark-200 mt-16 relative">
    ...
    <div class="container mx-auto max-w-5xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-3xl mx-auto">
            <?php
            // Only use the dark_theme_simplicity_approach_items from the Front Page Sections panel
            try {
                // Get approach items from customizer
                $json_data = get_theme_mod('dark_theme_simplicity_approach_items', '');
                
                // Safely decode JSON
                $approach_items = json_decode($json_data, true);
                
                // Check for JSON errors
                if (json_last_error() !== JSON_ERROR_NONE) {
                    // Log error
                    error_log('Section approach JSON error: ' . json_last_error_msg() . ' in: ' . substr($json_data, 0, 100));
                    $approach_items = array();
                }
                
                // Ensure we have a valid array to work with
                if (!is_array($approach_items)) {
                    $approach_items = array();
                }
            } catch (Exception $e) {
                // Log any unexpected exceptions
                error_log('Exception in approach section: ' . $e->getMessage());
                $approach_items = array();
            }
            ...
            if (!empty($approach_items) && is_array($approach_items)) {
                ...
                // Default approach items if none are set or data is invalid
