...
/**
 * ENHANCED Mobile Menu Debug Function - Priority Load
 */
function enhanced_mobile_menu_debug() {
    $file_path = get_template_directory() . '/assets/js/mobile-menu.js';
    $file_url = get_template_directory_uri() . '/assets/js/mobile-menu.js';
    
    // Enhanced debugging
    error_log('=== ENHANCED MOBILE MENU DEBUG ===');
    error_log('Theme directory: ' . get_template_directory());
    error_log('Theme directory URI: ' . get_template_directory_uri());
    error_log('Mobile menu file path: ' . $file_path);
    error_log('Mobile menu file URL: ' . $file_url);
    ...
    if (file_exists($file_path)) {
        ...
        error_log('File modification time: ' . date('Y-m-d H:i:s', filemtime($file_path)));
    }
    
    // Force enqueue with highest priority
    wp_enqueue_script(
        'mobile-menu-enhanced-debug',
        $file_url,
        array(),
    ...
    wp_add_inline_script('mobile-menu-enhanced-debug', '
        console.log("🚀 === ENHANCED MOBILE MENU DEBUG ===");
        console.log("✅ Script URL: ' . esc_js($file_url) . '");
        console.log("📁 Theme directory: ' . esc_js(get_template_directory()) . '");
        console.log("🌐 Theme URI: ' . esc_js(get_template_directory_uri()) . '");
        console.log("📱 File exists: ' . (file_exists($file_path) ? 'YES' : 'NO') . '");
        console.log("🔧 Timestamp: " + new Date().toLocaleTimeString());
        
        // Test mobile menu elements immediately
        document.addEventListener("DOMContentLoaded", function() {
            console.log("🎯 DOM Ready - Testing mobile menu elements...");
            ...
            console.log("📋 Mobile menu:", menu ? "✅ FOUND" : "❌ NOT FOUND");
            console.log("🎭 Overlay:", overlay ? "✅ FOUND" : "❌ NOT FOUND");
            
            if (toggle && menu) {
                console.log("🎉 All elements found! Mobile menu should work!");
            } else {
                console.error("⚠️ Missing mobile menu elements!");
            }
        });
    ');
}
add_action('wp_enqueue_scripts', 'enhanced_mobile_menu_debug', 999);

/**
 * Fallback mobile menu in case external file fails
 */
function fallback_mobile_menu() {
    wp_add_inline_script('jquery', '
        jQuery(document).ready(function($) {
            console.log("🔄 Fallback mobile menu initializing...");
            
            // Check if external script worked first
            setTimeout(function() {
                ...
                if (toggle.length && menu.length) {
                    ...
                    toggle.off("click.fallback").on("click.fallback", function(e) {
                        e.preventDefault();
                        e.stopPropagation();
                    ...
                    $("#mobile-menu-overlay").off("click.fallback").on("click.fallback", function() {
                        menu.addClass("hidden");
                        $(this).addClass("hidden");
                        $("body").css("overflow", "");
                        console.log("📕 Fallback: Menu closed via overlay");
                    });
                    ...
                    $(document).off("keydown.fallback").on("keydown.fallback", function(e) {
                        if (e.key === "Escape" && !menu.hasClass("hidden")) {
                            ...
                            $("body").css("overflow", "");
                            console.log("📕 Fallback: Menu closed via escape");
                        }
                    });
                    
                    console.log("✅ Fallback mobile menu ready!");
                } else {
                    console.error("❌ Fallback: Mobile menu elements not found!");
                }
            }, 500);
        });
    ');
}
add_action('wp_enqueue_scripts', 'fallback_mobile_menu', 1000);